(function(){
  ...
  var quote=document.getElementById('hitokoto'); if(quote&&quote.dataset.quoteEndpoint){fetch(quote.dataset.quoteEndpoint).then(function(r){return r.json()}).then(function(data){quote.textContent=data.hitokoto||''; if(data.id&&quote.dataset.quoteLinkBase){quote.onclick=function(){window.open(quote.dataset.quoteLinkBase+data.id,'_blank','noopener')}}}).catch(function(){quote.textContent=''})}
